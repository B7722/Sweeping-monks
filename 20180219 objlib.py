...
def add_cube(position, radius_X, radius_Y, radius_Z, color):
    return dict(type='cube', position=np.array(position), radius_X=np.array(radius_X), radius_Y = np.array(radius_Y), radius_Z=np.array(radius_Z), color=np.array(color), reflection=.5)
...
scene = [add_cube([.75, .1, 1.],[1.75,.1,1],[.75,1.1,1],[.75,.1,2],[0., 0., 1.]),
